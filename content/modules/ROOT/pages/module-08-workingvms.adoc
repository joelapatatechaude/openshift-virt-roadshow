=  Working with Virtual Machines and Applications (Optional)
...
. Click *Save* and *Repeat* the process for the VM *winweb02*.
...
. To verify connectivity to the database service via the external IP, use the terminal to the right to authenticate to the database at it's exposed LB address with the credentials below.
